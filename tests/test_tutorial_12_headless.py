...
# %% [markdown]
# ## 2. データ読み込み: 講義資料 (PDF) のテキスト抽出
#
# アップロードされた `week12_2025.pdf` を読み込み、テキストデータを抽出します。
# 機械にとって扱いやすいように、意味のまとまり（チャンク）ごとに分割する処理も重要ですが、
# 今回はシンプルにページごと、あるいは一定の文字数で分割して扱います。

# %%
# PDFファイルのパス (Colab等の環境に合わせて適宜変更してください)
# 修正: テスト用に親ディレクトリのファイルを指すか、実行ディレクトリを合わせる
pdf_path = "week12_2025.pdf"

if not os.path.exists(pdf_path):
    # If running from tests/ look up one level
...
# CPU execution fallback for testing: force cpu if needed, but SentenceTransformer handles it automatically usually.
...
# Testing check: Check if bitsandbytes is available, if not, careful with load.
# Actually, since we don't specify load_in_4bit, it shouldn't require bitsandbytes strictly unless we installed a version that hooks into it.
# We will use torch.float32 for CPU testing safety if device is cpu.
...
def retrieve_relevant_chunks(query, k=3):
    ...
    # 上位k個のインデックスを取得
    top_results = torch.topk(cos_scores, k=min(k, len(corpus_embeddings)))
...
print("=== RAG vs No-RAG 比較デモ開始 (Test Mode) ===")
